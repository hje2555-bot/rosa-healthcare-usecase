...
> *"가구를 만드는 제조업 회사 A에는 로봇을 이용해 가구를 만드는 자동화 공장이 있습니다. 자연재해가 발생해서 이 공장 로봇들에게 문제가 발생했습니다. 로봇들은 그동안 자신들이 공장에서 무슨 업무를 하고 있었는지 기억하지 못합니다. 모든 메모리가 날아갔기 때문이죠. … 여러분이 해결해야 할 문제는 현재 기억이 사라진 로봇들에게 각각 다른 기억을 제작하여 이식하는 것입니다."*

**제약**: 모든 학습 자원(프로그래밍, 자료구조, RAG, WORKFLOW) 자유 활용 / GPT 모델 자유 사용 / **로봇에 입력되는 데이터는 반드시 JSON 형식**.
...
원본 미션은 *가구 공장 자동화 로봇*의 메모리 복원이었다. 본 과제 팀은 같은 *메모리 생성 워크플로우* 의 **임상 가치가 훨씬 큰 적용 도메인** 을 식별했다 — **가정 돌봄 로봇 (LG CLOi for Home Care, 삼온)**.
...
본 프로젝트는 LG 현직자가 제시한 원본 미션을 다음과 같이 발전시켰다.
...
5. **실증 트랙**: 단일 공장 → LG CLOi 양산 + LG 글로벌 채널
...
- 5계층 매핑: [01-architecture-mapping.md](01-architecture-mapping.md)
